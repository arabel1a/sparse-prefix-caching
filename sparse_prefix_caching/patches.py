...
def chunk_gated_delta_rule_with_states(
    ...
    Mirrors `fla.ops.gated_delta_rule.chunk.chunk_gated_delta_rule_fwd` but keeps
    the internal `h[B, NT, H, K, V]` tensor instead of discarding it.
    ...
    # input_guard decorator on the autograd version makes inputs contiguous.
    q = q.contiguous()
    k = k.contiguous()
    v = v.contiguous()
    g = g.contiguous()
